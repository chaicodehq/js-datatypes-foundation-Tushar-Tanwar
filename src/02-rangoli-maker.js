...
export function extractRangoliCenter(design, start, end) {
  // Your code here
    if (typeof design !== 'string' || typeof start !== 'number' ||typeof end !== 'number' || !Number.isInteger(start) || !Number.isInteger(end)) {
...
export function makeRangoliBorder(char, length) {
  // Your code here
    if (typeof char !== 'string' || typeof length !== 'number' || !Number.isInteger(length) || length <= 0) {
    return '';
  }
